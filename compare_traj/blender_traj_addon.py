...
def VSLAMMappingFromBlender2DSO(translation, quaternion):
    ...
    mat_alignRotation = Euler([-math.pi, 0.0, 0.0],
                              'XYZ').to_matrix().to_4x4()  # This is necessary, because I have decided to rotate Blenders camera such that it "lies" in
        ...
        for frame in range(scene.frame_start, scene.frame_end + 1):
            ...
            if (scene.vslam_bool_export_ideal):
                if (scene.vslam_bool_export_timestamp):
                    ...
                    timestamp, translation.x, translation.y, translation.z, orientation.w, orientation.x, orientation.y,
                    orientation.z))
                else:
                    ...
                    translation.x, translation.y, translation.z, orientation.w, orientation.x, orientation.y,
                    orientation.z))

            # Calculate noise for translation (additive to position)
            noise = np.random.normal(scene.vslam_noise_translation_mean, scene.vslam_noise_translation_std_deviation,
                                     size=3)
            ...
            noise = np.random.normal(scene.vslam_noise_orientation_mean, scene.vslam_noise_orientation_std_deviation,
                                     size=3)
...
class VSLAMToolPanel(Panel):
    ...
    def draw(self, context):
        ...
        box.prop(context.scene, "vslam_automatic_gain_control_std_deviation",
                 text="Standard Deviation (image brightness scale)", expand=True)
        row = settings.row()
...
def register():
    ...
    bpy.types.Scene.vslam_output_directory = bpy.props.StringProperty(subtype='DIR_PATH',
                                                                      default=bpy.path.abspath("//"))
    bpy.types.Scene.vslam_camera = bpy.props.StringProperty(
        default=bpy.context.scene.camera.name if bpy.context.scene.camera is not None else '')  # TODO: Dont use string, but bpy.props.PointerProperty

    bpy.types.Scene.vslam_altered_camera = bpy.props.StringProperty(default="Camera_altered")
    bpy.types.Scene.vslam_noise_translation_mean = bpy.props.FloatVectorProperty(name="Mean",
                                                                                 description="Enter the mean for gaussian noise",
                                                                                 default=(0.0, 0.0, 0.0), unit="LENGTH",
                                                                                 subtype="XYZ")
    bpy.types.Scene.vslam_noise_translation_std_deviation = bpy.props.FloatVectorProperty(name="Standard deviation",
                                                                                          description="Enter the standard deviation for gaussian noise",
                                                                                          default=(0.01, 0.01, 0.01),
                                                                                          unit="LENGTH", subtype="XYZ")
    bpy.types.Scene.vslam_noise_orientation_mean = bpy.props.FloatVectorProperty(name="Mean",
                                                                                 description="Enter the mean for gaussian noise",
                                                                                 default=(0.0, 0.0, 0.0),
                                                                                 unit="ROTATION", subtype="XYZ")
    bpy.types.Scene.vslam_noise_orientation_std_deviation = bpy.props.FloatVectorProperty(name="Standard deviation",
                                                                                          description="Enter the standard deviation for gaussian noise",
    ...
    bpy.types.Scene.vslam_automatic_gain_control_std_deviation = bpy.props.FloatProperty(name="AGC Standard deviation",
                                                                                         description="Enter the standard deviation for gaussian noise being added to normal 1.0 exposure",
                                                                                         default=0.02, unit="TIME")
    bpy.types.Scene.vslam_motion_blur_duration = bpy.props.FloatProperty(name="Motion Blur Shutter duration in frames",
                                                                         description="Enter the time in frames between shutter open and close",
                                                                         default=0.1, min=0.01, max=2.0,
                                                                         subtype="FACTOR")
    bpy.types.Scene.vslam_rolling_shutter_factor = bpy.props.FloatProperty(name="Rolling Shutter factor",
                                                                           description="Enter the percentage of how much rolling shutter there should be in relation to motion blur time (0 = no rolling shutter, 1 = only rolling shutter for the complete motion blur duration)",
    ...
    bpy.types.Scene.vslam_dso_scale_factor = bpy.props.FloatProperty(name="DSO: Scale down images",
                                                                     description="Scale images by factor", default=0.25)
