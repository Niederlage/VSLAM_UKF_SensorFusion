...
class LIEKF_Filter():
    ...
    def iekfPropagation(self, chi_i, bias_i, P_i, u_i, Q_i, dt_i):
        ...
        F_i = np.eye(N_P)
        wx = self.lg.hat_operator(omega_i)
        ax = self.lg.hat_operator(acc_i)

        F_i[:3, :3] = np.eye(3) - wx * dt_i + wx @ wx * dt_i ** 2 / 2

        F_i[3:6, :3] = (ax @ wx + wx @ ax) * dt_i ** 2 / 2 - ax * dt_i
        F_i[3:6, 3:6] = np.eye(3) - wx * dt_i + wx @ wx * dt_i ** 2 / 2

        F_i[6:9, :3] = -ax * dt_i ** 2 / 2
        F_i[6:9, 3:6] = np.eye(3) * dt_i - wx * dt_i ** 2
        F_i[6:9, 9:12] = np.eye(3) - wx * dt_i + wx @ wx * dt_i ** 2 / 2
        ...
        Q_t = block_diag(np.zeros((3, 3)), Q_i * dt_i)
        P_predict = F_i @ P_i @ F_i.T + Q_t * dt_i
